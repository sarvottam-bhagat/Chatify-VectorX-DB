...
# Initialize VectorX collection lazily
...
# Configure logfire to suppress warnings and avoid pandas dependencies
...
def get_db_stats():
    ...
    try:
        ...
        # Get unique URLs - handle potential missing 'url' field
        urls = set()
        domains = set()
        last_crawled_times = []
        ...
        for meta in results["metadatas"]:
            if meta and isinstance(meta, dict):
                ...
                if crawled_at:
                    last_crawled_times.append(crawled_at)
...
async def process_url(url: str):
    ...
    try:
        ...
        with progress_container.container():
            ...
            try:
                doc_count = len(get_vectorx_collection().get()["ids"])
                st.success(
                    f"""
                ✅ Processing complete! 
            ...
            if st.button(
                ...
                try:
                    ...
                    if all_ids:  # Only attempt to delete if there are documents
                        get_vectorx_collection().delete(ids=all_ids)
                    # Reset session state
                    st.session_state.processing_complete = False
                    st.session_state.urls_processed = set()
                    st.session_state.messages = []
                ...
                with st.expander("📝 Suggested Questions", expanded=False):
                    if existing_data and existing_data["doc_count"] > 0:
                        ...
                        if st.session_state.suggested_questions is None:
                            st.session_state.suggested_questions = (
                                generate_contextual_questions(get_vectorx_collection())
                            )
